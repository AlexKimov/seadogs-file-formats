...
class NoeUserDialog(ctypes.Structure): 
    def __init__(self, title = "", default_extension = "", filter_string = "", \
        ...
        self.ofn.Flags = OFN_ENABLESIZING | OFN_PATHMUSTEXIST | \
...
class NoeUserProgressBar(noewin.NoeUserControlBase):
    def __init__(self, noeParentWnd, name, controlId, x, y, width, height, commandMethod):
        ...
        self.hWnd = ctypes.windll.user32.CreateWindowExW(0, "MSCTLS_PROGRESS32", self.name, \
            self.style, x, y, width, height, noeParentWnd.hWnd, self.controlId, \
            noeParentWnd.hInst, 0)   
...
class NoeUserStaticImage(noewin.NoeUserControlBase):
    def __init__(self, noeParentWnd, name, controlId, x, y, width, height, commandMethod):
        ...
        self.hWnd = ctypes.windll.user32.CreateWindowExW(0, "STATIC IMAGE", self.name, \
            self.style, x, y, width, height, noeParentWnd.hWnd, self.controlId, \
            noeParentWnd.hInst, 0)              
...
class NoeUserTreeView(noewin.NoeUserControlBase):
    def __init__(self, noeParentWnd, name, controlId, x, y, width, height, commandMethod):
        ...
        self.name = name
        self.style = noewin.WS_VISIBLE | noewin.WS_CHILD | noewin.WS_BORDER \
            | TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS
        self.hWnd = ctypes.windll.user32.CreateWindowExW(0, "SysTreeView32", self.name, \
            self.style, x, y, width, height, noeParentWnd.hWnd, self.controlId, \
            noeParentWnd.hInst, 0) 
    ...
    def insertItem(self, text, parentItem = None):    
        ...
        return ctypes.windll.user32.SendMessageW(self.hWnd, TVM_INSERTITEM, 0, \
            ctypes.byref(tvins))	           
    ...
    def getParent(self, item):
        return ctypes.windll.user32.SendMessageW(self.hWnd, TVM_GETNEXTITEM, \
            TVGN_PARENT, item)
    ...
    def getItemText(self, item):
        ...
        tvi.hItem = item        
        ctypes.windll.user32.SendMessageW(self.hWnd, TVM_GETITEM, 0, \
            ctypes.byref(tvi))   
...
class  NoeUserGroupBox(noewin.NoeUserControlBase):   
    def __init__(self, noeParentWnd, name, controlId, x, y, width, height, commandMethod):
        ...
        self.hWnd = ctypes.windll.user32.CreateWindowExW(0, "Button", self.name, \
            self.style, x, y, width, height, noeParentWnd.hWnd, self.controlId, \
            noeParentWnd.hInst, 0) 
...
class NoeUserWindowExt(noewin.NoeUserWindow):
    ...
    def createTreeView(self, x, y, width, height, name = "", commandMethod = None):
        if self.hWnd:        
            newTreeView = NoeUserTreeView(self, name, \
                self.currentControlId, x, y, width, height, commandMethod)
    ...
    def createGroupBox(self, x, y, width, height, name = "", commandMethod = None):
        if self.hWnd:        
            newGroupBox = NoeUserGroupBox(self, name, \
                self.currentControlId, x, y, width, height, commandMethod)
